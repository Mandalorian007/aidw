...
class OneshotCommand(BaseCommand):
    ...
    def get_progress_steps(self) -> list[ProgressStep]:
        """Return the progress steps for the oneshot workflow.

        Returns:
            List of six steps: analyze issue, create plan, implement, test,
            document, and create PR
        """
        return [
    ...
    async def run_workflow(
        ...
        """Run the complete end-to-end workflow from issue to ready PR.

        Workflow steps:
        1. Analyze issue - Renders prompt with issue context
        2. Create plan - Claude Code generates implementation plan file
        3. Implement - Claude Code implements all code changes
        4. Test - Claude Code writes and runs tests
        5. Document - Claude Code updates documentation
        6. Create PR - Pushes branch and creates a ready-for-review PR (not draft)

        This combines plan + build into a single autonomous workflow that
        produces a complete, ready-to-merge PR.

        Args:
            session: Database session tracking this workflow
            context: Workflow context with issue, repo, and plan path information
        ...
        Returns:
            Dictionary with pr_number and pr_url of the created PR
...
"""
    ...
    async def execute_manual(self, repo: str, issue: int, instruction: str = "") -> None:
        """Execute oneshot command manually from CLI or script.

        Args:
            repo: Repository in owner/name format
            issue: Issue number to implement in one shot
            instruction: Optional additional instruction to guide the workflow
        """
        cmd = ParsedCommand(
